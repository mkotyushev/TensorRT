# Implementing Hardshrink activation in TensorRT with a custom plugin


**Table Of Contents**
- [Implementing activation in TensorRT with a custom plugin](#implementing-hardshrink-activation-in-tensorrt-with-a-custom-plugin)
...
This sample, sampleHardshrink, converts a model trained on the `MNIST dataset` in Open Neural Network Exchange (ONNX) format to a TensorRT network and runs inference on the network.
This model was trained in PyTorch and it contains Hardshrink activations instead of Relu activations.<br/>
Model with Hardshrink layers training script in PyTorch could be found here: [link](TODO)<br/>
Original model with usual Conv layers: [link](https://github.com/pytorch/examples/tree/master/mnist)

[Hardshrink](https://pytorch.org/docs/stable/generated/torch.nn.Hardshrink.html) is activation layer which applies the hard shrinkage function element-wise.
This plugin can be found at `TensorRT/plugin/hardshrinkPlugin`. Additional information about the layer and plugin implementation can be found at `TensorRT/plugin/hardshrinkPlugin/README.md`

ONNX is a standard for representing deep learning models that enables models to be transferred between frameworks.

## How does this sample work?

This sample creates and runs a TensorRT engine on an ONNX model of MNIST trained with Hardshrink activations. It demonstrates how TensorRT can parse and import ONNX models with Hardshrink activations.

Specifically, this sample:
- [Converts the ONNX model with Hardshrink activations to a TensorRT network](#converting-the-onnx-model-to-a-tensorrt-network)
- [Builds an engine with Hardshrink activations](#building-an-engine)
- [Runs inference using the generated TensorRT network](#running-inference)

Description below and most of source code of `sampleOnnxMnistHardshrink.cpp` is based on `TensorRT/samples/opensource/sampleOnnxMnistCoordConvAC` sample.
...
The Activation layer implements element-wise activation functions. Specifically, this sample uses the Activation layer with the type `kHARDSHRINK`.
...
1.  Compile this sample by running `make` in the `<TensorRT root directory>/samples/sampleOnnxMnistHardshrink` directory. The binary named `sample_onnx_mnist_hardshrink` will be created in the `<TensorRT root directory>/bin` directory.
	```
	cd <TensorRT root directory>/samples/sampleOnnxMnistHardshrink
	make
	```
...
2.  Run the sample to build and run the MNIST engine from the ONNX model.
	```
	./sample_onnx_mnist_hardshrink [-h or --help] [-d or --datadir=<path to data directory>] [--useDLACore=<int>] [--int8 or --fp16]
	```

3.  Verify that the sample ran successfully. If the sample runs successfully you should see output similar to the following:
	```
	&&&& RUNNING TensorRT.sample_onnx_mnist_hardshrink # ./sample_onnx_mnist_hardshrink
	----------------------------------------------------------------
	Input filename:   data/mnist/mnist_with_hardshrink.onnx
    ...
    &&&& PASSED TensorRT.sample_onnx_mnist_hardshrink # ./sample_onnx_mnist_hardshrink
	```
...
```
Usage: ./sample_onnx_mnist_hardshrink [-h or --help] [-d or --datadir=<path to data directory>] [--useDLACore=<int>]
...
- [PyTorch documentation](https://pytorch.org/docs/stable/generated/torch.nn.Hardshrink.html)
